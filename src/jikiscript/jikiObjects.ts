...
type RawConstructor = (executionContext: ExecutionContext, object: Instance, ...args: JikiObject[]) => void;
...
export class Getter {
  constructor(
    public readonly visibility: Visibility,
    public readonly fn: (executionContext: ExecutionContext, object: Instance) => JikiObject
  ) {}
}
export class Setter {
  constructor(
    public readonly visibility: Visibility,
    public readonly fn: (executionContext: ExecutionContext, object: Instance, value: JikiObject) => JikiObject
  ) {}
}
...
export class Class {
  ...
  public instantiate(executionContext: ExecutionContext, args: JikiObject[]): Instance {
  ...
  public addGetter(name: string, visibility: Visibility, fn?: (_: ExecutionContext, object: Instance) => JikiObject) {
    if (fn === undefined) {
...
export function unwrapJikiObject(value: any): any {
  ...
  if (value instanceof Dictionary) {
    return Object.fromEntries([...value.value.entries()].map(([key, value]) => [key, unwrapJikiObject(value)]));
  }
  if (isArray(value)) {
  ...
  if (typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, value]) => [key, unwrapJikiObject(value)]));
  }
