...
function push(_: ExecutionContext, list: Jiki.List, element: Jiki.JikiObject): Jiki.List {
...
function concat(_: ExecutionContext, list1: Jiki.List, list2: Jiki.List): Jiki.List {
...
function keys(_: ExecutionContext, dict: Jiki.Dictionary): Jiki.List {
  ...
  return new Jiki.List(
    Array.from(dict.value.keys() as IterableIterator<string>).map((key: string) => new Jiki.String(key))
  );
}

function min(_: ExecutionContext, num1: Jiki.Number, num2: Jiki.Number): Jiki.Number {
...
function max(_: ExecutionContext, num1: Jiki.Number, num2: Jiki.Number): Jiki.Number {
  verifyType(num1, Jiki.Number, "number", 1);
  verifyType(num2, Jiki.Number, "number", 2);
...
function verifyType(arg: any, targetClass: any, targetType: string, argIdx: number) {
