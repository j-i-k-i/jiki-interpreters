...
export function describeMethodCallExpression(
  ...
  const argSteps = expression.args.map((arg, idx) => describeExpression(arg, result.args[idx], context)).flat();
  ...
  const fnDesc = generateMethodDescription(expression, result, methodName, context);
  const fnCallDesc = argNames.length > 0 ? `${methodName}(${argNames})` : `${methodName}()`;

  const actionCodeTag = codeTag(fnCallDesc, expression.location);
  const who = expression.object.type === "ThisExpression" ? "this object's" : "the object's";
