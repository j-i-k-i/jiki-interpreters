...
function describeRepeat(frameContext: RepeatStatement, frameResult: EvaluationResultRepeatStatement): Description {
  ...
  const steps = [
    `<li>Jiki increased his internal counter for this loop to <code>${frameResult.iteration}</code>, checked ${codeTag(
      `${frameResult.iteration} &le; ${formatJikiObject(frameResult.count.jikiObject)}`,
      frameContext.count.location
