...
function describeReturnWithValue(frame: FrameWithResult, context: DescriptionContext) {
  const frameContext = frame.context as ReturnStatement;
  ...
  steps = [
    ...steps,
    `<li>Jiki put ${codeTag(value, frameContext.expression!.location)} in the return chute.</li>`,
    finalStep,
  ];
