...
function describeShortCircuitedExpression(
  ...
  steps = [
    ...steps,
    `<li>Jiki saw the left side of the <code>${expression.operator.lexeme}</code> was <code>${formatJikiObject(
...
function describeFullExpression(
  ...
  return [
    ...
    `<li>Jiki evaluated ${codeTag(
      ...
      expression.location
    )} and determined the result was ${codeTag(result.jikiObject, expression.location)}.</li>`,
  ];
}
