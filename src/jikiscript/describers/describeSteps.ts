...
export function describeExpression(
  ...
  if (expression instanceof FunctionCallExpression) {
    return describeFunctionCallExpression(expression, result as EvaluationResultFunctionCallExpression, context);
  }
  if (expression instanceof MethodCallExpression) {
    return describeMethodCallExpression(expression, result as EvaluationResultMethodCallExpression, context);
  }
  if (expression instanceof VariableLookupExpression) {
    return describeVariableLookupExpression(expression, result as EvaluationResultVariableLookupExpression, context);
  }
  if (expression instanceof InstantiationExpression) {
    return describeInstantiationExpression(expression, result as EvaluationResultInstantiationExpression, context);
  }
