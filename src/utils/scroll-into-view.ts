...
export function scrollIntoView(): void {
  ...
  const docsSideMenuTrigger = document.getElementById("side-menu-trigger") as HTMLInputElement | null;
...
const collectAndScroll = (axis: ScrollAxis) => {
  document.querySelectorAll<HTMLElement>('[data-scrollable-container="true"]').forEach(container => {
    const elements = Array.from(container.querySelectorAll<HTMLElement>(`[data-scroll-into-view="${axis}"]`));
...
function scrollElementsIntoView(elements: HTMLElement[], container: HTMLElement, axis: ScrollAxis): void {
...
function scrollToElementWithinContainer(element: HTMLElement, container: HTMLElement, axis: ScrollAxis) {
  const elementRect = element.getBoundingClientRect();
  const containerRect = container.getBoundingClientRect();

  const { size, scroll, start, end } = axisProps[axis];

  const isElementVisible = elementRect[start] >= containerRect[start] && elementRect[end] <= containerRect[end];
