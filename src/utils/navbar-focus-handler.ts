export function handleNavbarFocus(): void {
  const navElements = document.querySelectorAll<HTMLElement>(".nav-element-focusable");
  ...
  navElements.forEach(navElement => {
    ...
    navElement.addEventListener("focus", () => {
      if (currentMouseOverElement) {
        const dropdown = currentMouseOverElement.querySelector<HTMLElement>(".nav-element-dropdown");
        if (dropdown) {
  ...
  function removeFocusFromOtherElements(currentElement: HTMLElement) {
    navElements.forEach(otherElement => {
      if (otherElement !== currentElement) {
        otherElement.blur();
        const focusableChildren = otherElement.querySelectorAll<HTMLElement>('[tabindex="0"]');
