...
function describeNotExpression(
  ...
  const resBool = (result.operand.immutableJikiObject || result.operand.jikiObject) as JS.JSBoolean;
  ...
  steps = [
    ...
    )} is ${codeTag(result.immutableJikiObject || result.jikiObject, expression.location)}.</li>`,
  ];
  return steps;
}
...
function describeMinusExpression(
  ...
  const resNum = (result.operand.immutableJikiObject || result.operand.jikiObject) as JS.JSNumber;
  let steps = describeExpression(expression.operand, result.operand, context);
  steps = [
    ...steps,
    `<li>JavaScript evaluated that ${codeTag(
