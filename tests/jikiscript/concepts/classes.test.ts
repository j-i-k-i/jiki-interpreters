...
describe("parse", () => {
  ...
  describe("constructor statement", () => {
    test("naked", () => {
      ...
      `);
      expect((stmts[0] as ClassStatement).body[0]).toBeInstanceOf(ConstructorStatement);
      const constructorStatement = (stmts[0] as ClassStatement).body[0] as ConstructorStatement;
    ...
    test("args", () => {
      ...
      expect((stmts[0] as ClassStatement).body[0]).toBeInstanceOf(ConstructorStatement);
      const constructorStatement = (stmts[0] as ClassStatement).body[0] as ConstructorStatement;
      expect(constructorStatement.parameters.map(p => p.name.lexeme)).toEqual(["arg1", "arg2"]);
      expect(constructorStatement.body).toBeEmpty();
    });
    ...
    test("with this.setter", () => {
      ...
      const setPropertyStatement = constructorStatement.body[0] as SetPropertyStatement;
    ...
    test("with this.changer", () => {
      ...
      expect(constructorStatement.body[1]).toBeInstanceOf(ChangePropertyStatement);
      const changePropertyStatement = constructorStatement.body[1] as ChangePropertyStatement;
      ...
      expect((changePropertyStatement.value as LiteralExpression).value).toBe(15);
    });
  });
  describe("method statement", () => {
    test("public naked", () => {
      ...
      `);
      expect((stmts[0] as ClassStatement).body[0]).toBeInstanceOf(MethodStatement);
      const methodStatement = (stmts[0] as ClassStatement).body[0] as MethodStatement;
    ...
    test("public args", () => {
      ...
      expect(methodStatement.accessModifier.lexeme).toBe("public");
      expect(methodStatement.name.lexeme).toBe("foobar");
      expect(methodStatement.parameters.map(p => p.name.lexeme)).toEqual(["arg1", "arg2"]);
      expect(methodStatement.body).toBeEmpty();
    });
    test("private naked", () => {
      const stmts = parse(`
  ...
  describe("property statement", () => {
    test("public", () => {
      ...
      `);
      expect((stmts[0] as ClassStatement).body[0]).toBeInstanceOf(PropertyStatement);
      const propertyStatement = (stmts[0] as ClassStatement).body[0] as PropertyStatement;
