...
describe("parse", () => {
  describe("expression", () => {
    ...
    test("chained methods", () => {
      ...
      expect((innerCallExpr.object as VariableLookupExpression).name.lexeme).toBe("foo");
...
describe("execute", () => {
  test("no args", () => {
    ...
    Person.addMethod("name", "", "public", function (_: ExecutionContext, object: Jiki.Instance) {
  ...
  test("with args", () => {
    const Person = new Jiki.Class("Person");
    Person.addConstructor(function (_: ExecutionContext, object: any, name: Jiki.JikiObject) {
      object.setField("name", name);
    });
    Person.addMethod(
      "name_char",
