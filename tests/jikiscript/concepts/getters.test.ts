...
describe("parse", () => {
  ...
  test("chained methods", () => {
    ...
    expect((innerAccessorExpr.object as VariableLookupExpression).name.lexeme).toBe("foo");
  });
  test("chained properties", () => {
    ...
    expect((accessorExpr.object as VariableLookupExpression).name.lexeme).toBe("foo");
  });
});
