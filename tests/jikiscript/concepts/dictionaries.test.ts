...
describe("parse", () => {
  describe("init", () => {
    ...
    test("single element", () => {
      ...
      expect((mapExpr.elements.get("title") as LiteralExpression).value).toBe("Jurassic Park");
    });
    ...
    test("nested", () => {
      const stmts = parse('log {"title": "Jurassic Park", "director": { "name": "Steven Spielberg" } }');
      expect(stmts).toBeArrayOfSize(1);
      expect(stmts[0]).toBeInstanceOf(LogStatement);
      const logStmt = stmts[0] as LogStatement;
      expect(logStmt.expression).toBeInstanceOf(DictionaryExpression);
      ...
      expect(mapExpr.elements.get("title")).toBeInstanceOf(LiteralExpression);
      expect((mapExpr.elements.get("title") as LiteralExpression).value).toBe("Jurassic Park");
      expect(mapExpr.elements.get("director")).toBeInstanceOf(DictionaryExpression);
      const nestedMapExpr = mapExpr.elements.get("director") as DictionaryExpression;
      expect(nestedMapExpr.elements.size).toBe(1);
      expect(nestedMapExpr.elements.get("name")).toBeInstanceOf(LiteralExpression);
      expect((nestedMapExpr.elements.get("name") as LiteralExpression).value).toBe("Steven Spielberg");
    });
  });
