...
describe("arithmetic concepts", () => {
  describe("integer literals", () => {
    test("simple integer", () => {
      ...
      expect(frames[0].description).toContain("42");
    });
  ...
  describe("floating point literals", () => {
    test("simple float", () => {
      ...
      expect(error).toBeNull();
      expect(frames).toBeArrayOfSize(1);
      expect(frames[0].description).toContain("3.14");
    });
    ...
    test("scientific notation", () => {
      ...
      expect(error).toBeNull();
      expect(frames).toBeArrayOfSize(1);
      expect(frames[0].description).toContain("150");
    });
    ...
    test("negative scientific notation", () => {
      ...
      expect(error).toBeNull();
      expect(frames).toBeArrayOfSize(1);
      expect(frames[0].description).toContain("0.002");
    });
  });
});
